...
- ✅ 全栈 TypeScript 开发（前端 Vue3 + 后端 Node.js，共享类型定义，开发效率高）
...
- **前端:** Vue 3.5+ + TypeScript 5.x + Vite + Tailwind CSS
- **后端:** Node.js (v18+) + TypeScript + Express + Prisma ORM
- **数据库:** MySQL 8.0+
- **AI服务:** 阿里云百炼（通义千问 qwen-turbo），使用 Node.js SDK
- **图片服务:** Unsplash API / Pexels API（免费）

### 1.2 后端基础搭建

#### 任务 1: Node.js 后端项目初始化

**优先级:** P0（必须）

**实现内容:**

- 创建 Node.js 项目 (`npm init`)
- 配置 TypeScript 环境 (`tsconfig.json`)
- 安装核心依赖 (Express, Prisma, TypeScript, ts-node)
- 配置开发环境热重载 (nodemon 或 ts-node-dev)
- 配置 ESLint + Prettier 代码规范
- 添加通义千问 SDK (`dashscope`)
- API 接口基础封装 (路由、统一响应处理)

**技术要点:**

- 全栈 TypeScript (共享 types 目录)
...
- `backend/src/config/config.ts` - 环境变量配置
- `backend/.env` - 敏感配置 (不提交)
...
);Prisma Schema 参考 (`backend/prisma/schema.prisma`):**

```prisma
model Ingredient {
  id        Int      @id @default(autoincrement())
  name      String   @unique
  category  String?  // 蔬菜/肉类...
  unit      String?  // 克/个...
  isCommon  Boole (Node.js SDK)
...
- 集成 `dashscope` npm 包
- 封装 AI 调用服务
- 实现结构化 Prompt（通过 System Message 约束 JSON 输出）
- 错误处理和重试机制
- 数据清洗与类型转换

**技术要点:**

- 使用 `@google/generative-ai` 或 `dashscope` (本例使用通义千问)
- TypeScript 接口定义 AI 响应结构
- 异步处理 (async/await)

**文件涉及:**

- `backend/src/services/aiService.ts` - AI 服务逻辑
- `backend/src/utils/aiPrompt.ts` - Prompt 管理
- `backend/src/types/recipe.ts` - 菜谱相关类型定义

**AI Prompt 设计 (TypeScript):**

```typescript
const prompt = `
你是一个专业的中餐厨师助手。用户提供了以下食材：${ingredients.join(', ')}
...
- `backend/src/services/ingredientService.ts` - 食材服务
- `backend/src/services/recipeService.ts` - 菜谱服务

#### 任务 3: 通义千问集成 (Node.js SDK)
...
- `backend/src/controllers/recipeController.ts` - 菜谱控制器
- `backend/src/controllers/ingredientController.ts` - 食材控制器
- `backend/src/routes/index.ts` - 路由定义
...
export const searchRecipeImage = async (recipeName: string): Promise<string> => {
    // 调用 Unsplash API 搜索图片
    const apiKey = process.env.UNSPLASH_ACCESS_KEY;
    const url = `https://api.unsplash.com/search/photos?query=${encodeURIComponent(recipeName)} food&client_id=${apiKey}`;
    
    try {
        const response = await axios.get(url);
        // 返回第一张图片的 URL，如果没有则返回默认图
        return response.data.results?.[0]?.urls?.small || 'default-food-image-url';
    } catch (error) {
        console.error('Image search failed:', error);
        return 'default-food-image-url';
    }
};       // 返回第一张图片的 URL
    }
}
