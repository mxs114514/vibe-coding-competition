...
**接口地址：** `GET /ingredients/common`

**说明：** 获取预设的常用食材列表，按分类分组，用于快速选择。
...
```json
{
  ...
  "data": {
    "categories": {
      "蔬菜": ["白菜", "番茄", "黄瓜", "土豆", "胡萝卜", "青椒", "洋葱", "西兰花"],
      "肉类": ["猪肉", "鸡肉", "牛肉", "羊肉", "鸡蛋", "鸭肉"],
      "海鲜": ["虾", "鱼", "蟹", "贝类", "鱿鱼"],
      "主食": ["米饭", "面条", "馒头", "饺子皮", "面粉"],
      "调味料": ["盐", "油", "酱油", "醋", "糖", "料酒", "生抽", "老抽"]
    }
  }
}
```
...
| **食材** | GET    | /ingredients                      | 获取标准食材库         | ❌   |
|          | GET    | /ingredients/common               | 获取常用食材列表       | ❌   |
|          | GET    | /user-ingredients                 | 获取用户冰箱食材       | ✅   |
